...
有两种构建类型的语法：[`interface`](https://www.typescriptlang.org/play?e=83#example/types-vs-interfaces) 和 [`type`](https://www.typescriptlang.org/play?e=83#example/types-vs-interfaces)。你应该更喜欢使用 `interface`，`type` 只会在需要特定功能时使用。
